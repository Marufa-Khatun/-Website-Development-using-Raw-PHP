              ...
              <tr>
               ...
               <?php foreach($datas as $data): ?>
                  <tr>
                      ...
                      <td><?=$data['box_icon']?></td>
                      <td><?=$data['box_color']?></td>
                      <td> <span class="badge bg-<?= $data['status'] == 1 ? "success": "warning" ?>"> <?= $data['status'] == 1 ? "Active": "Deactive" ?></span></td>
                    
                    
                      <td>
                          <a href="<?=adminUrl()?>/service/single_service.php?id=<?=$data['id']?>" class="btn btn-sm btn-secondary">View</a>
                          <a href="<?=adminUrl()?>/service/edit_service.php?id=<?=$data['id']?>" class="btn btn-sm btn-primary">Edit</a>
                          <a href="<?=adminUrl()?>/service/del_service.php?id=<?=$data['id']?>" class="btn btn-sm btn-danger">Delete</a>
                          <a href="<?=adminUrl()?>/service/status.php?id=<?=$data['id']?>" class="btn btn-sm btn-<?=$data['status'] == 1 ? "warning": "success" ?>">
